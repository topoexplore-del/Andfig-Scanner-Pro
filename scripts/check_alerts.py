"""
ANDFIG SCANNER PRO — Multi-Layer Alert System v18.0
...
# ═══ HELPERS ═══════════════════════════════════════════════════
import math
# Ventana de blackout pre-earnings (días). Configurable por entorno.
EARNINGS_BLACKOUT_DAYS = int(os.environ.get('EARNINGS_BLACKOUT_DAYS', '5'))

def live_price(ticker):
    """Cotización en vivo (o casi) de Yahoo al momento de la alerta."""
    try:
        import yfinance as yf
        t = yf.Ticker(str(ticker).strip().upper().replace('.', '-').replace('/', '-').rstrip('*'))
        fi = t.fast_info
        p = fi.get('last_price') if hasattr(fi, 'get') else getattr(fi, 'last_price', None)
        if not p:
            p = fi.get('lastPrice') if hasattr(fi, 'get') else getattr(fi, 'lastPrice', None)
        return round(float(p), 2) if p else None
    except Exception:
        return None

def classify_freshness(a):
    """Compara el precio EN VIVO contra las zonas de la señal y dictamina si
    aún es operable. Evita perseguir señales que ya corrieron a los TP."""
    z = a.get('zones') or {}
    entry, tp1 = z.get('entry'), z.get('tp1')
    atr = a.get('atr') or (tp1 - entry if tp1 and entry else None)
    if not entry or not tp1 or not atr:
        return None
    max_chase = round(entry + 0.5 * atr, 2)
    lp = live_price(a['ticker'])
    a['live'] = lp
    a['max_chase'] = max_chase
    if lp is None:
        st = {'icon': '❔', 'label': 'SIN COTIZACIÓN', 'note': 'no se pudo validar el precio en vivo — verifica antes de operar'}
    elif lp >= tp1:
        st = {'icon': '🔴', 'label': 'TARDE', 'note': f'ya alcanzó TP1 (${tp1}) — solo informativa, NO perseguir'}
    elif lp > max_chase:
        st = {'icon': '🟠', 'label': 'EXTENDIDA', 'note': f'sobre el límite de persecución ${max_chase} — espera pullback hacia ${entry}'}
    elif lp > entry:
        st = {'icon': '🟡', 'label': 'CERCA', 'note': f'operable con disciplina hasta ${max_chase} (límite de persecución)'}
    else:
        st = {'icon': '🟢', 'label': 'EN ZONA', 'note': 'precio dentro/bajo la zona de entrada — señal plenamente vigente'}
    a['status'] = st
    return st

def json_safe(o):
    """NaN/Infinity no son JSON válido — los convierte en null recursivamente."""
...
def validate_all(r):
    ...
    result = {'ticker': r.get('ticker','?'), 'name': r.get('name','?'), 'price': r.get('close',0),
              ...
              'earn_days': r.get('earn_days'), 'earn_date': r.get('earn_date'), 'asof': r.get('asof'),
              'mos': r.get('mos'), 'rs_rank': r.get('rs_rank'), 'atr': r.get('atr'),
    ...
    # Entry zones — ATR-based: el stop respira con la volatilidad real del activo.
    close = r.get('close',0) or 1
    atr = r.get('atr')
    if atr and atr > 0:
        entry = round(close - 0.3*atr, 2)        # leve pullback
        result['zones'] = {'entry': entry,
                           'tp1': round(entry + 1.0*atr, 2),
                           'tp2': round(entry + 2.0*atr, 2),
                           'sl':  round(entry - 1.5*atr, 2)}
    else:
        result['zones'] = {'entry': round(close*0.985,2), 'tp1': round(close*1.03,2),
                           'tp2': round(close*1.06,2), 'sl': round(close*0.97,2)}
...
def build_telegram_msg(confirmed, near_miss, timestamp, regime_info=None):
    """Build Telegram message with Markdown."""
    msg = f"🎯 *ANDFIG SCANNER PRO*\n{timestamp}\n"
    if regime_info:
    ...
    for a in confirmed:
        ...
        bq = a.get('buffett'); mos = a.get('mos'); rsr = a.get('rs_rank')
        if bq is not None:
            mos_txt = f" | MOS: {mos:+.0f}%" if mos is not None else ""
            msg += f"🏰 Calidad: {a.get('buffett_v','')} ({bq}/100){mos_txt}\n"
        if rsr is not None:
            msg += f"📈 Fuerza relativa vs SPY: percentil {rsr}\n"
        ed = a.get('earn_days')
        if ed is not None and ed <= 14:
            msg += f"📅 ATENCIÓN: earnings en {ed} día(s) ({a.get('earn_date','')})\n"
        st = a.get('status')
        if st:
            msg += f"⏱ Señal del cierre {a.get('asof','—')} | Ahora: ${a.get('live','—')}\n"
            msg += f"{st['icon']} *{st['label']}*: {st['note']}\n"
        # Tamaño de posición: riesgo 1% del capital hasta el SL
        try:
            risk_ps = z['entry'] - z['sl']
            if risk_ps > 0:
                shares_10k = int(100 / risk_ps)  # 1% de $10,000 = $100
                msg += f"⚖️ Tamaño (riesgo 1%): ≈{shares_10k} acc. por cada $10k de capital\n"
...
def build_email_html(confirmed, near_miss, timestamp):
    ...
    <div style="background:#12131a;border:1px solid #2a2b36;border-radius:8px;padding:20px;margin-bottom:16px;text-align:center">
        <h1 style="color:#00e676;margin:0;font-size:22px;letter-spacing:2px">🎯 ANDFIG SCANNER PRO</h1>
    ...
    html += f"""<div style="text-align:center;padding:16px;color:#5a5c66;font-size:10px;border-top:1px solid #2a2b36">
        <p style="margin:0">ANDFIG SCANNER PRO v18.0 · Solo LONG · 4-Layer Validation</p>
...
def main():
    ...
    print(f"ANDFIG SCANNER PRO — Alert Check · {timestamp}\n{'='*55}")

    # ── Filtro de régimen de mercado (Faber SMA200) ──
    regime_info = snapshot.get('market_regime') or {}
    regime = regime_info.get('regime', 'NEUTRAL')
    print(f"  {regime_info.get('icon','🟡')} Régimen de mercado: {regime} — {regime_info.get('advice','')}")
    if regime == 'BEAR':
        print("  🔴 Mercado bajista: alertas de COMPRA suspendidas (sistema LONG-only).")
        alerts_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'alerts.json')
        with open(alerts_path, 'w') as f:
            json.dump(json_safe({'checked_at': timestamp, 'regime': regime_info, 'confirmed': 0,
                       'signals': [], 'watchlist': [],
                       'note': 'Alertas suspendidas: SPY bajo SMA200 (régimen bajista).'}), f, indent=2, allow_nan=False)
        return
    ...
    for gn, stocks in snapshot.get('groups', {}).items():
        for r in stocks:
            ...
            # Blackout de earnings: no se compra a ciegas justo antes del reporte
            ed = r.get('earn_days')
            if res['all_passed'] and ed is not None and ed <= EARNINGS_BLACKOUT_DAYS:
                res['all_passed'] = False
                res['failed'].append(f"Earnings en {ed}d (blackout)")
                res['layers_passed'] = min(res['layers_passed'], 3)
            # En régimen NEUTRAL solo pasan señales con fuerza relativa alta
            if regime == 'NEUTRAL' and res['all_passed']:
                rsr = res.get('rs_rank')
                if rsr is not None and rsr < 60:
                    res['all_passed'] = False
                    res['failed'].append('RS (régimen neutral)')
                    res['layers_passed'] = 3
            if res['all_passed']:
    ...
    # ── Deduplicación: no reenviar la misma señal el mismo día ──
    state_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'alerts_state.json')
    ...
    if confirmed and not new_signals:
        print(f"  ℹ️ {len(confirmed)} señal(es) activas pero ya notificadas hoy — no se reenvía.")
    state = {today: sorted(already | {a['ticker'] for a in confirmed})}  # solo se guarda hoy
    ...
    alerts_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'alerts.json')
    with open(alerts_path, 'w') as f:
        json.dump(json_safe({'checked_at': timestamp, 'regime': regime_info, 'confirmed': len(confirmed),
                   'signals': [{'ticker':a['ticker'],'composite':a['composite'],'prob':a['prob'],
                               'buffett':a.get('buffett'),'mos':a.get('mos'),'rs_rank':a.get('rs_rank'),
                               'asof':a.get('asof'),'live':a.get('live'),'status':a.get('status'),'max_chase':a.get('max_chase'),
    ...
    if new_signals:
        print("  ⏱ Validando frescura con precio en vivo...")
        for a in new_signals:
            st = classify_freshness(a)
            if st:
                print(f"    {a['ticker']}: señal del cierre {a.get('asof','?')} | ahora ${a.get('live','?')} → {st['icon']} {st['label']}")
        # Email
        if smtp_user and smtp_pass and to_email:
            try: send_email(new_signals, near_miss, to_email, smtp_user, smtp_pass, timestamp)
            except Exception as e: print(f"  ❌ Email error: {e}")
        else:
            print(f"\n  ⚠️ SMTP not configured. Set SMTP_USER + SMTP_PASS + ALERT_EMAIL secrets.")

        # Telegram
        if tg_token and tg_chat:
            try:
                tg_msg = build_telegram_msg(new_signals, near_miss, timestamp, regime_info)
